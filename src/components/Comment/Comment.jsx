...
function Comments({ movieId }) {
  ...
  useEffect(() => {
    ...
    fetch(`${process.env.REACT_APP_SERVER_URL}/user/name`)
      .then(response => response.json())
  ...
  const handleSubmit = (event) => {
    ...
    fetch(`${process.env.REACT_APP_SERVER_URL}/comments/post`, {
      method: 'POST',
      headers: {
  ...
  return (
    <div className="mx-auto p-5 bg-gray-900 text-gray-200">
      ...
      <div className="space-y-4 mb-6">
        {comments.map(comment => (
          <div key={comment.id} className="flex items-start space-x-3">
            <svg className="h-10 w-10 rounded-full bg-gray-700 text-white" fill="currentColor" viewBox="0 0 24 24">
      ...
      <form onSubmit={handleSubmit} className="flex items-center gap-2">
        ...
        />
        <button type="submit" className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white font-bold rounded-r-md">Submit</button>
      </form>
    </div>
  );
}
